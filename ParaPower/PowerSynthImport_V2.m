function maxtemp = PowerSynthImport_V2(jsondata)
...
% psjson = fileread('./TestLayers/0.json');
% psjson = fileread('./TestLayers3/PSData_JSON.json');
...
[time, temp] = get_global_max(Results, 'Thermal');
maxtemp = max(temp);
...
% Visualize('Temperature', Results.Model, 'State', Tprnt(:,:,:,end))
% figure 
% Visualize('Model', Results.Model)
% zlim = [0, 0.005]
% S1=scPPT('PSMI',PSMI); %Initialize object
% setup(S1,[]);
% % tic
% [Tprnt, T_in, MeltFrac,MeltFrac_in]=S1([InitTime ComputeTime(1:min(StepsToEstimate,length(ComputeTime)))]);  %Compute states at times in ComputeTime (S1 must be called with 1 arg in 2017b)
% % EstTime=toc;
...
pp_mats = {'Cu', 'SAC405', 'AlN', 'SiC', 'Al', 'AlN', 'Cu', 0};
