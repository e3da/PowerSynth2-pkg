...
# MATLAB is now optional
InstallMatlab=true
if [ ! -f "$MatlabRoot/VersionInfo.xml" ]; then
	if [ -f "$MatlabSearch" ]; then
		MatlabRoot=`dirname "$MatlabSearch"`
	else
		echo "WARNING: Matlab is not found at $MatlabRoot. Skipping matlabengine installation."
		InstallMatlab=false
	fi
fi

if [ "$InstallMatlab" = true ]; then
