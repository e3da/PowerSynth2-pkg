...
from Standard_Trace_Model import GroundPlane, Velement, write_to_file,Element,Run,Init
...
global_var = {
    'mdk_lib':'./layer_stacks',
    'model_lib': '../../work/Sample_Designs/RS_Models',
    'ws_dir':"./WS",
    'mat_lib':"../../MDK/Materials.csv",
    'Width_Range':[0.1,15],
    'Length_Range':[0.1,50],
...
def generate_rsmdl_lib():
    ...
    for layer_stack_name in csv_files:
        ...
        if clean_up:
            ...
            remove_files(ws_dir +'/*.inp')
            remove_files(ws_dir +'/*.csv')
            remove_files(ws_dir +'/*.log')
...
def build_RS_model(frange = None,model_input = None,wdir =None):
    ...
    for i in range(len(frange)):
        ...
        RAC_input.export_RAW_data(dir = global_var['ws_dir'],k=0,freq = str(frange[i]))
    ...
    for i in range(len(frange)):
        ...
        LAC_input.export_RAW_data(dir = global_var['ws_dir'],k=1,freq = str(frange[i]))
...
def build_and_run_trace_sim(**kwags):
    ...
    ''' Run FastHenry'''
    print(fname)
    cmd = fasthenry_env + " " + fasthenry_option +" "+fname + f">{wdir}/out" + str(cpu) +".log &"
    os.system(cmd)
...
def process_output(**kwags):
    ...
    #print stdout,stderr
    ''' Output Path'''
    outname=os.path.join(f"Zc"+str(cpu)+ '.mat')
